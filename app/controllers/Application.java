...
//				System.out.println("van " + i + " partida " + line);
				try {
					...
					b.close();
					if(line1.equalsIgnoreCase("{\"status\": {\"message\": \"Service unavailable\", \"status_code\": 503}}") || line1.equalsIgnoreCase("{\"status\": {\"message\": \"Rate limit exceeded\", \"status_code\": 429}}")){
						System.out.println(line);
						HttpResponse homePage = WS
								.url("https://las.api.pvp.net/api/lol/las/v2.2/match/"
										+ line
										+ "?includeTimeline=true&api_key=7916da78-982f-472e-8d93-7e62949b86f7")
								.get();
						JsonParser parser = new JsonParser();
						JsonObject obj = (JsonObject) parser.parse(homePage.getString());
						File file_json = new File("./LAS/"+line+".json");
						PrintWriter print = new PrintWriter(file_json);
						print.write(homePage.getString());
						print.close();	
					}
					line = br.readLine();
					tiempo = 500;
